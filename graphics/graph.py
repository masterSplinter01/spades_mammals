...
#dict {t: average  = number of pairs / number of UNIQUE edges}
t_edge_pairs_count_average = dict()

for key in t_edge_pairs:
	if len(t_edge_pairs[key]) == 0:
		t_edge_pairs_count_average[key] = 0.0
	else:
		t_edge_pairs_count_average[key] = len(t_edge_pairs[key]) / t_uniq_edges_count[key]

#sorting to build plot
t_edge_pairs_count_average = sorted(t_edge_pairs_count_average.items())

x, y = zip(*t_edge_pairs_count_average)

plt.plot(x, y)
plt.title("average")
plt.savefig("t_edge_pairs_count_average.png")

#print(t_edge_pairs_count_average)
